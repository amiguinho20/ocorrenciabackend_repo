...
@RequestScoped
public class ConsultarOcorrenciaDAO {

	private Logger logger =  LogManager.getLogger(ConsultarOcorrenciaDAO.class);
	...
	@Log
	public String consultar(String idDelegacia, String anoBo, String numBo)
	{
		...
		try
		{
			...
			logger.info("conexao pega com o autocommit[" + conn.getAutoCommit() + "].");
			conn.setAutoCommit(false);
			logger.info("conexao configurada para autocommit false.");
			
			int intIdDelegacia = Integer.parseInt(idDelegacia);
			int intAnoBo = Integer.parseInt(anoBo);
			int intNumBo = Integer.parseInt(numBo);
			
			retorno = consultar(intIdDelegacia,  intAnoBo,  intNumBo, conn);
		}
		catch(Exception e)
		{
	 ...
	 * @return
	 */
	private String consultar(int idDelegacia, int ano, int numero, Connection conn){
	...
	private String prepararConsultarIterar(Connection conn, String atributoJson, String prepare, String queryPreparada, Object... chaves) throws SQLException 
	{
		...
		catch (SQLException e)
		{
			...
			msg.append("Erro no prepararConsultarIterar: " + e.getMessage() + " ");
			msg.append("Na query [" + queryPreparada + "] ");
			msg.append("Com as chaves [" + params.toString() + "] ");
			msg.append("Do atributo [" + atributoJson + "] ");
			msg.append("Do prepare [" + prepare + "] ");
