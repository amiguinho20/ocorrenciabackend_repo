package br.com.fences.ocorrenciardobackend.config.listener;
...
import br.com.fences.ocorrenciardobackend.config.AppConfig;

/**
 * Depende da configuracao no servidor da propriedade JNDI 'ocorrenciardobackend'
 *
 */
@WebListener
//@ApplicationScoped
public class ConfiguracaoDaAplicacaoContextListener implements ServletContextListener {
	...
	@Resource(mappedName="ocorrenciardobackend")
    ...
    private void configurarLogDaAplicacao()
    {
		try
		{
	        ...
	        if (logDiretorio != null && !logDiretorio.trim().isEmpty())
	        {
		        ...
		        try {
					appender = new DailyRollingFileAppender(patternLayout, logDiretorio + "/ocorrenciardobackend.log", "'.'yyyy-MM-dd'.log'");
